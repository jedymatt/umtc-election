<div>
    <div class="flex flex-row justify-center items-baseline gap-4">
        ...
        <div x-data>
            <x-button-primary x-on:click="$dispatch('createEvent')">New Event
            </x-button-primary>
        </div>
        <script>
            window.addEventListener('createEvent', function () {
                Swal.fire({
                    icon: 'info',
    ...
    <div class="mt-4 flex flex-row items-baseline gap-4">
        ...
        <div class="flex-grow flex justify-start items-baseline gap-4">
            @foreach ($electionTypes as $electionType)
                <input class="" type="radio" name="currentElectionType"
                       wire:model="currentElectionTypeId"
                       id="currentElectionType.{{ $electionType->id }}" value="{{ $electionType->id }}">
                <label class=""
        ...
        <form wire:submit.prevent="createElection" method="post">
            ...
            <div class="mt-4">
                ...
                <textarea name="description" id="description"
                          class="w-full rounded-md" wire:model.defer="form.description"></textarea>
            </div>
            <div class="mt-4">
            ...
            <div class="mt-4">
                ...
                <input type="datetime-local" id="end_at" name="end_at" class="rounded-md" wire:model.defer="form.end_at"
                       value="{{ old('end_at') }}"/>
                @error('end_at')
                <p class="text-sm text-red-600">{{ $message }}</p>
                @enderror
