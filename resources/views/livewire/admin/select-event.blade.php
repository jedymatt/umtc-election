<div x-data="{ isOpen: false }" class="relative w-full" x-on:click.away="isOpen=false">
        ...
        <div class="w-full py-2 rounded-md shadow-sm border bg-white">
            ...
            <ul class="max-h-48 border-t mt-2 overflow-y-scroll">
                @foreach ($events as $event)
                    <li x-on:click="$wire.selectEvent({{ $event->id }});isOpen = false"
                        class="p-2 text-sm select-none hover:bg-blue-600 hover:text-white">{{ $event->title }}</li>
                @endforeach
            </ul>
        </div>
